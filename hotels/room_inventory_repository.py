...
class RoomInventoryRepository:
    def __init__(self):
        hotel_id = 12
        self.hotel_inventory = dict([(hotel_id, self.__produce_by_hotel_id(hotel_id))])

    @staticmethod
    def __produce_by_hotel_id(hotel_id):
        return [RoomInventory(hotel_id, 0, RoomType(BedType.QUEEN, 2), 20)]

    def get_room_inventory(self, hotel_id, sequence_number) -> Optional[RoomInventory]:
        if self.hotel_inventory[hotel_id] is not None:
            for inventory in self.hotel_inventory[hotel_id]:
